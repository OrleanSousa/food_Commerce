...
platesRouter.delete("/:plateId", async (req, res) => {
    const { success, statusCode, body } = await platesControllers.deletePlate(req.params.plateId);

    res.status(statusCode).send({ success, statusCode, body });
});

platesRouter.put("/:plateId", async (req, res) => {
    const { success, statusCode, body } = await platesControllers.updatePlates(req.params.plateId, req.body);
